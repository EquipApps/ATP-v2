...
namespace EquipApps.Mvc.Internal
{
    public class MvcActionDescriptorProvider : IActionProvider
    {
        ...
        public MvcActionDescriptorProvider(
            IApplicationModelService appModelFactory,
            ILogger<MvcActionDescriptorProvider> logger,
        ...
        public void OnProvidersExecuting(ActionDescriptorContext context)
        {
            logger.LogTrace(nameof(OnProvidersExecuting));

            //-- 
            logger.LogTrace($"call {nameof(IApplicationModelService.GetApplicationModel)}");
        ...
        public void OnProvidersExecuted(ActionDescriptorContext context)
        {
            //-- Ничего не делаем!
            logger.LogTrace(nameof(OnProvidersExecuting));
        }
        ...
        private IEnumerable<ControllerTestCase> ToTestSuitOrderByNumber(IEnumerable<ControllerModel> controllerModels)
        {
            ...
            //foreach (var controller in controllerNulNumber)
            //{
            //    //-- Увеличивем счетчик пока не найдем свободный
            //    while (controllers.ContainsKey(index))
            //        index++;
        ...
        //TODO: Юнит тест проверка порядка!
        private IEnumerable<ControllerTestCase> ToTestCase(ControllerTestSuit testSuit)
        {
            var controllerModel = testSuit.ControllerModel;
            var backgroundMode = controllerModel.Background;
            var modelBinder = controllerModel.ModelBinder;

            //-- 1) Нету привязки? создаем одиночный TestCase
            if (modelBinder == null)
            {
                yield return new ControllerTestCaseEmpty();
                yield break;
            }

            //-- 2) Не получилось привязаться к данным? пропускаем.
            var resultBinding = modelBinder.Bind(testSuit, 1);
            if (!resultBinding.IsModelSet)
            {
                logger.LogError(
                    resultBinding.Exception,
            ...
            if (!resultBinding.IsMany)
            {
                // TODO: Проверить в юнит тесте. Background всегда ли отлична от нуля когда есть привязка?
                // TODO: Если один тест кейс, может не использовать индекс?
                var testCase = new ControllerTestCase(controllerModel.Background, startIndex);
                testCase.DataContext = resultBinding.Model;
            ...
            for (int i = 0, index = startIndex; i < bindingResults.Length; i++, index++)
            {
                ...
                var testCase = new ControllerTestCase(backgroundMode, index);
                testCase.DataContext = bindingResult.Model;
